...
def test_circle_fit():
    ...
    test_points_noise = [(expect_r * np.cos(phi) + x_noise[i] + expect_c[1],
                          expect_r * np.sin(phi) + y_noise[i] + expect_c[0])
    ...
    test_points = [(expect_r * np.cos(phi) + 0 * x_noise[i] + expect_c[1],
                    expect_r * np.sin(phi) + 0 * y_noise[i] + expect_c[0])
                   for i, phi in enumerate(
